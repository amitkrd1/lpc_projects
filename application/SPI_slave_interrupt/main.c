...
void SPI0_IRQHandler(void)
{
  uint32_t int_status;
//	
  int_status=Chip_SPI_GetIntStatus(LPC_SPI0);
	
	//printf("%d \r\n",int_status);
  //if( Chip_SPI_GetIntStatus(LPC_SPI0)){
...
static void SPI_SlaveTask(void *pvParameters)
{
...
//				(TaskHandle_t *) NULL);
	...
	xTaskCreate(SPI_SlaveTask, "vTaskSPISlave",
				configMINIMAL_STACK_SIZE, NULL, (tskIDLE_PRIORITY + 1UL),
				(TaskHandle_t *) NULL);
	/* Start the scheduler */
	vTaskStartScheduler();
